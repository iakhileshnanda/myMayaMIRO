"""
Maya MIRO Backend - Flask Application Factory
"""
...
def create_app(config_class=Config):
    ...
    if should_log_startup:
        logger.info("=" * 50)
        logger.info("Maya MIRO Backend starting...")
        logger.info("=" * 50)
    ...
    @app.before_request
    def log_request():
        logger = get_logger('mayamiro.request')
    ...
    @app.route('/health')
    def health():
        return {'status': 'ok', 'service': 'Maya MIRO Backend'}

    if should_log_startup:
        logger.info("Maya MIRO Backend startup complete")
